...
export default function PostflopPanel({positions,preflopActions,stacks,heroSeat}:{positions:Position[];preflopActions:PlayerAction[];stacks:number[];heroSeat:number}){
  ...
  const [solverCapability,setSolverCapability]=useState<{backend:"texassolver"|"dcfr"|"counterfactual";maxPlayers:number}>({backend:"texassolver",maxPlayers:2});
  useEffect(()=>{const controller=new AbortController();fetch("/api/solver",{signal:controller.signal,cache:"no-store"}).then(response=>response.ok?response.json():Promise.reject()).then(value=>{if(value&&["texassolver","dcfr","counterfactual"].includes(value.backend)&&Number.isInteger(value.maxPlayers))setSolverCapability(value);}).catch(()=>{});return()=>controller.abort();},[]);
  ...
  return <section className="postflop-builder">
    ...
    <button className="local-solver-button" disabled={!solverReady||solving} onClick={runLocalSolver}>{solving?"SOLVING…":solverCapability.backend==="counterfactual"?"COUNTERFACTUAL SOLVER 실행":solverCapability.backend==="dcfr"?"LOCAL DCFR EV 실행":"LOCAL TEXASSOLVER 실행"}</button><small className="local-solver-help">FAST PREVIEW · {solverCapability.backend==="counterfactual"?`외부 backend · 최대 ${solverCapability.maxPlayers}명`:solverCapability.backend==="dcfr"?"Heads-up · OOP street root · solver counterfactual EV":"Heads-up · street root 및 중간 액션 노드 · Hero OOP/IP"} · 상위 8개 weighted class.</small>
